...
function DeletedOrders() {
	...
	const handleRestoreOrders = () => {
		if (window.confirm("Do you really want to clear all orders?")) {
			fetch(`${ENDPOINT}/restore_orders`, {
				method: "POST",
			})
				.then((response) => response.json())
				.then((data) => {
					if (data.status === "All orders restored") {
	...
	return (
		<div className="App">
			<h1>Order Display</h1>
			<button onClick={handleRestoreOrders} className="restore-button">
				Restore Orders
			</button>
			<table>
				<thead>
